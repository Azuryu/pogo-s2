// ==UserScript==
// @id           s2check@azuryu
// @name         PoGO Tools, With Azu Sync
// @category     Layer
// @namespace    https://github.com/Azuryu/pogo-s2/raw/main/s2check.user.js
// @homepageURL  https://github.com/Azuryu/pogo-s2/
// @version      0.100.0
...
<p><label for="webhook">Webhook</label>&nbsp;<input type="text" id="webhook"></p>
            ...
            const webhookInput = div.querySelector('#webhook');
            webhookInput.value = settings.webhook;
            webhookInput.addEventListener('change', function (e) {
                settings.webhook = webhookInput.value;
                saveSettings();
            });
        ...
        function drawCell(cell, color, weight, opacity) {
            ...
            const region = L.geodesicPolyline([corners[0], corners[1], corners[2], corners[3], corners[0]], {
                fill: false,
                color: color,
                opacity: opacity,
                weight: weight,
        ...
        const IS_POGO = true; //decides over render styling
        ...
        // Send changed data to webhook
        function sendToDatabase(guid, type, deleteData = false) {
            ...
            if(settings.webhook) {
                ...
                if (type === 'gyms' || type === 'gym') {
                    data.gyms = {};
                    data.gyms[guid] = gyms[guid];
                    data.gyms = cleanUpExtraData(data.gyms);
                }

                if (type === 'pokestops' || type === 'pokestop') {
                    data.pokestops = {};
                    data.pokestops[guid] = pokestops[guid];
                    data.pokestops = cleanUpExtraData(data.pokestops);
                }

                if (type === 'notpogo') {
                    data.notpogo = {};
                    data.notpogo[guid] = notpogo[guid];
                    data.notpogo = cleanUpExtraData(data.notpogo);
                }

                console.log('Send to database', data);
                ...
                fetch(settings.webhook, requestOptions)
        ...
        /**
         * Sync data between storage and database
         */
        thisPlugin.syncWithDatabase = function () {

            if(!settings.webhook) {
                alert('No webhook set');
                return false;
            }

            //save a backup just in case
            thisPlugin.optExport();
            ...
            thisPlugin.optAlert('Request PoGo Sync data.');
            fetch(settings.webhook, requestOptions)
                ...
                .then(result => {
                    ...
                    Object.keys(mappings).forEach(category => {
                        ...
                        let {local, db} = mappings[category];

                        //If dataDatabase is not empty
                        if (db) {

                            //Iterate trough each stop/gym from database
                            Object.keys(db).forEach(id => {

                                //If local data exists, only overwrite when date is lower
                                if (local[id]) {

                                    //In case local data does not have a timestamp yet, assign it a small number
                                    if (!local[id]._updated_at) {
                                        local[id]._updated_at = -1;
                                    }

                                    //Compare timestamps
                                    if (parseInt(local._updated_at) < parseInt(db._updated_at)) {
                                        updated++;
                                        Object.assign(local[id], db[id]);
                                    }

                                }

                                //If local data does not exist, database data can just be assigned
                                else {
                                ...
                                //Add data to cleanup array to remove entries that appear in both gyms and stops afterwards
                                states.push({id: id, category: category});
                    ...
                    states.forEach(state => {
                        ...
                        if (state.category !== 'pokestops') {
                            delete pokestops[state.id];
                        }

                        if (state.category !== 'gyms') {
                            delete gyms[state.id];
                        }

                        if (state.category !== 'notpogo') {
                            delete notpogo[state.id];
                        }

                    });

                    saveStorage();
                    thisPlugin.optAlert('Merged PoGo Sync data');
                    thisPlugin.optAlert('Added: ' + added);
                    thisPlugin.optAlert('Updated: ' + updated);
        ...
        thisPlugin.smashToDatabase = function () {
            if (confirm('Are you really sure? This assumes you own the most correct dataset.')) {
                thisPlugin.optAlert('Send all data to database.');
                sendAllToDatabase();
                thisPlugin.optAlert('All data has been sent.');
            }
        };
        ...
        thisPlugin.addPortalpogo = function (guid, lat, lng, name, type) {
            ...
            const obj = {'guid': guid, 'lat': lat, 'lng': lng, 'name': name, '_updated_at': -1};
        ...
        */
        ...
        thisPlugin.pogoActionsDialog = function () {
            const content = `<div id="pogoSetbox">
				...
				<a onclick="window.plugin.pogo.syncWithDatabase();return false;" title="Sync with database">Sync with Database</a>
				<a onclick="window.plugin.pogo.smashToDatabase();return false;" style="color: red" title="Smash all data to database, this will override gym/stop states of existing database entries.">Smash to Database</a>
				</div>`;
